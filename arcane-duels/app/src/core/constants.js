(function (A) {
  ...
  const SCHOOL_ICON_GLYPHS = Object.freeze({
    fire: "🔥",
    water: "💧",
    air: "🌪️",
    nature: "🍃",
  ...
  A.schoolIconMarkup = function schoolIconMarkup(id, className = "school-icon-svg") {
    const key = SCHOOL_ICON_GLYPHS[id] ? id : "fire";
    return `<span class="${className} school-icon-emoji school-icon-${key}" aria-hidden="true">${SCHOOL_ICON_GLYPHS[key]}</span>`;
  };
